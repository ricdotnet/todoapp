    ...
    <div class="text-center">
        ...
        <div class="w-2/3 md:w-1/2 lg:w-1/3 mx-auto">
            ...
            <div v-if="showSuccessMessage" class="text-center animate__bounceIn">
                ...
                <a href="/login" class="p-5 w-full flex 
                bg-green-500
                hover:bg-green-600
                active:bg-green-700
                rounded-md
            ...
            <form v-if="!showSuccessMessage" @submit.prevent="doRegister()">
                <input v-model="username" class="w-full mb-5 p-5 bg-blue-600 bg-opacity-25 outline-none rounded-md shadow" placeholder="username" autocomplete="off">
                <input v-model="email" class="w-full mb-5 p-5 bg-blue-600 bg-opacity-25 outline-none rounded-md shadow" placeholder="email" autocomplete="false">
                <input v-model="password" class="w-full mb-5 p-5 bg-blue-600 bg-opacity-25 outline-none rounded-md shadow" placeholder="password" type="password" autocomplete="off">
            
                <button class="p-5 w-full bg-green-500 font-bold
                        hover:bg-green-600
                        active:bg-green-700
                        focus:outline-none
                        shadow rounded-md">Register</button>
...
const url = "https://ricr.dev/api/todoapp.php?action=register";
    ...
    methods: {
        ...
        async doRegister() {
            ...
            if(this.username == "") {
                 ...
                 */
                await axios.get(url+"&username="+this.username+"&password="+this.password+"&email="+this.email)
                    .then(response => {
                        ...
                        if(response.data == "username_exists") {
                            ...
                            this.errorMessage = "Something went wrong here. Please refresh the page."
                            this.showErrorMessage = true;
